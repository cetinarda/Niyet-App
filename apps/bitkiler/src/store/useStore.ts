...
const STORAGE_KEYS = {
  PROFILE: '@sakinbitkiler_profile',
  DAILY: '@sakinbitkiler_daily',
  ARCHIVE: '@sakinbitkiler_archive',
  STATS: '@sakinbitkiler_stats',
  LANGUAGE: '@sakinbitkiler_language',
  VIEWED: '@sakinbitkiler_viewed_today',
};
...
export function useSakinHayvanStore() {
  ...
  // Keşfet'te bir kart (bitki detay) AÇILINCA okuma say (kullanıcı isteği:
  // "7 bitkiye bakınca Yol Başlangıcı rozeti yansın"). AYNI KART GÜNDE 1 KEZ sayılır.
  // Farklı 7 kart açınca totalReadings 7'ye ulaşır ve rozet yanar. AsyncStorage'dan
  // okuyup yazar (paylaşılan store olmadığı için ekranlar ayrı kopya tutar).
